...
export default function BrandProductPage() {
    ...
    const { getBrandProducts, brandProducts } = useContext(context);
  ...
  useEffect(() => {
    getBrandProducts('title',"0","100000",'1',BrandId)
  }, [BrandId])
  ...
  async function handleSortChange(event) {
    ...
    await getBrandProducts(sortingParam, newMinPrice, newMaxPrice,pageNum,BrandId);
  ...
  async function handlePageClick(pageNumber) {
    ...
    const newMinPrice = minPrice === "" ? "0" : minPrice;
    const newMaxPrice = maxPrice === "" ? "1000000" : maxPrice;
    await getBrandProducts(sortingParam, newMinPrice, newMaxPrice, pageNumber,BrandId);
  ...
  return (
    <>
      ...
      <div className="row gy-5">
        <Product Products={brandProducts} />
      </div>
      <div className="d-flex pt-5 justify-content-center align-items-center ">
        {brandProducts.metadata ? (
          <>
            {brandProducts.metadata.prevPage ? (
              <span
                className="btn me-2 bg-transparent text-muted border-black"
                onClick={() => handlePageClick(brandProducts.metadata.prevPage)}
              >
                {brandProducts.metadata.prevPage}
              </span>
            ) : (
            ...
            {brandProducts.metadata.currentPage ? (
              <span
                className="btn me-2 bg-main text-white"
                onClick={() => handlePageClick(brandProducts.metadata.currentPage)}
              >
                {brandProducts.metadata.currentPage}
              </span>
            ) : (
            ...
            {brandProducts.metadata.nextPage ? (
              <span
                className="btn me-2 bg-transparent text-muted border-black"
                onClick={() => handlePageClick(brandProducts.metadata.nextPage)}
              >
                {brandProducts.metadata.nextPage}
              </span>
            ) : (
